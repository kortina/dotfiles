...
class S3ScreenshotHandler(RegexMatchingEventHandler):
    ...
    def _upload_file(self, new_filepath):
        conn = boto.connect_s3(self._aws_id(),
                               self._aws_secret())
    ...
    def _mv_and_upload_file(self, filepath):
        ...
        if self._match_groups(os.path.basename(filepath)) is None:
            return
        ...
        notify_osx("Uploaded Screenshot to S3",
                   "{0}: Copied link to clipboard.".format(filename))

    def _http_link(self, filename):
        return "https://s3.amazonaws.com/{0}/{1}".format(self._bucket_name(),
                                                         filename)
